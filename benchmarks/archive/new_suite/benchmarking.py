...
def build_eigprob(dom, n_fb_basis, fb_strat='singular_weighted', n_fs_basis=0, fs_dist=1.0,
    ...
    if n_fb_basis > 0:
        fb_pts_per_seg = geometry.pts_per_seg(dom, fb_basis, mult=n_bdry_mult)
    else:
        fb_pts_per_seg = 0
    if n_fs_basis > 0:
        n_bdry = n_bdry_mult*len(fs_basis)
        fs_pts_per_seg = np.ceil(n_bdry*dom.seg_lens/dom.seg_lens.sum()).astype(int)
    else:
        fs_pts_per_seg = 0
    pts_per_seg = fb_pts_per_seg + fs_pts_per_seg
