...
`reg_ngon_6` is scored against a CORRECTED spectrum; see the note on `_CORRECTED` below.
"""
import warnings
...
# `benchmarks/suite/run/reference_values.REFERENCE['reg_ngon_6']` is short a mode: it lists
# 32.45185751 once and then 37.49135288. That eigenvalue is a genuine DOUBLE -- both members of
# the cluster, L^2-orthonormal, certify to 9.2 Moler--Payne digits, matching the known-genuine
# double at 18.13167787 (9.1 each) and the simple lambda_1 (9.0); two orthonormal eigenfunctions
# at one lambda force multiplicity >= 2. The two-term Weyl count agrees (N(37.0) = 4.75, i.e.
# five modes below 37, not four), and a regular hexagon's D6 symmetry has two 2-dimensional
# irreps, so doubles there are generic rather than surprising.
#
# This is the same defect `benchmarks/basis_lab/HEURISTICS.md` found in `reg_ngon_8` (a mode
# missing from its table, caught by Faber--Krahn), from the same `symmetry(...)` reference
# generator. The table is NOT edited here: it is generated by `benchmarks.suite.emit` and feeds
# certified results elsewhere, so it needs a regeneration, not a patch -- the same call
# `docs/todo.md` records for `ellipse_eigs(a=2)`. Recorded there; corrected here so that this
# test measures the solver rather than the reference.
_CORRECTED = {
    'reg_ngon_6': np.array([7.15533913, 18.13167787, 18.13167787, 32.45185751, 32.45185751,
                            37.49135288, 47.62936577, 52.63789014, 60.10511209, 60.10511209]),
}


def _truth(key):
    return _CORRECTED.get(key, reference_eigs(key, max(KS))[0])
